...
function TaskCard({ task, onClick }: any) {
  return (
    <motion.div draggable onDragStart={e => e.dataTransfer.setData("taskId", task.id)} onClick={onClick} layoutId={task.id} className="bg-zinc-950 border border-zinc-800 p-5 rounded-lg hover:border-zinc-600 transition-all cursor-grab active:cursor-grabbing group shadow-md relative overflow-hidden">
      <div className="flex justify-between items-start mb-4">
        <span className="text-[9px] font-mono text-zinc-600 group-hover:text-blue-500 transition-colors uppercase font-bold tracking-tighter">{task.id}</span>
        <span className="text-[10px] font-mono font-bold text-blue-400 bg-blue-500/5 px-2 py-0.5 rounded border border-blue-500/10">{task.score}</span>
      </div>
      <h4 className="text-xs font-bold text-zinc-200 leading-snug group-hover:text-white mb-5">{task.title}</h4>
